...
public class Datacenter extends SimEntity {
	...
	/** The battery mapping from File Transfer Time */
	private final double batteryMappingFactor = 0.02;
	 ...
	 * 
	 */
	public static Wifi intitWifi() {
		Wifi objWifi = null;
		try
		{
			objWifi = Wifi.getInstance("Mobile_Wifi");
		}
		catch(Exception ex)
		{
			ex.printStackTrace();
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			// time to transfer the files
			double fileTransferTime = predictFileTransferTime(cl.getRequiredFiles(), cl);
			...
			Object[] batteryData = new Object[2];
			batteryData[0] = fileTransferTime * batteryMappingFactor;
			batteryData[1] = cl;
			
			// Added By Ridhi Sharma
			send(objBat.getId(), 0, CloudSimTags.Battery_DEC, batteryData);
	...
	protected double predictFileTransferTime(List<String> requiredFiles, Cloudlet cl) {
		double time = 0.0;
