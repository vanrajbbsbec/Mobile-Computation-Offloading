...
public class Cloudlet {
	...
	//Added By RIDHI SHARMA
	
	/** The battery life before execution of cloudlet */
	private double beforeBatteryLife;
	
	/** The battery life after execution of cloudlet */
	private double afterBatteryLife;
	
	public double requiredFileSize;
	...
	//Added By RIDHI SHARMA
	/** The battery Object */
	private Battery objBattery;
	
	/** The priority field  */
	@SuppressWarnings("unused")
	private int priority;
	
	protected final int maxPriority = 5;
	protected final int minPriority = 1;
	...
	public Cloudlet(
		...
		objBattery = Battery.intitBattery();
		setBeforeBatteryLife(objBattery.getBatteryLife());
		//setPriority(pPriority);
	}

	private void setPriority(int pPriority) {
		if(pPriority < 1)
		{
	...
	public Cloudlet(final int i, final long length, final int pesNumber, final long fileSize, final long outputSize,
			final UtilizationModel utilizationModel, final UtilizationModel utilizationModel2, final UtilizationModel utilizationModel3,
			final int priority2) {
		userId = -1;
		status = CREATED;
		this.cloudletId = i;
		...
		// Cloudlet length, Input and Output size should be at least 1 byte.
				this.cloudletLength = Math.max(1, cloudletLength);
				this.cloudletFileSize = Math.max(1, fileSize);
				this.cloudletOutputSize = Math.max(1, outputSize);
				resList = new ArrayList<Resource>(2);
				index = -1;
				this.record = false;
	...
	public double getBeforeBatteryLife()
	{
		return beforeBatteryLife;
	}
	
	public double getAfterBatteryLife()
	{
		return afterBatteryLife;
	}
	
	public void setBeforeBatteryLife(double batteryValue)
	{
		if(batteryValue < 0)
		{
			this.beforeBatteryLife = 0;
		}
		else
		{
			this.beforeBatteryLife = batteryValue;
		}
	}
	
	
	public void setAfterBatteryLife(double batteryValue)
	{
		if(batteryValue < 0)
		{
			this.afterBatteryLife = 0;
		}
		else
		{
			this.afterBatteryLife = batteryValue;
		}
	}
	
	/*public double getRequiredFileSize(Cloudlet cl) {
		int clId = cl.getCloudletId();
		for (String string : requiredFiles) {
			getCloudletFileSize()
		}
